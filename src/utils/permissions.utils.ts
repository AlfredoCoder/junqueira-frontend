...
export const USER_PERMISSIONS: Record<string, UserPermissions> = {
  ...
  // Professor - Acesso completo à gestão acadêmica e estudantes
  'professor': {
    canAccessStudentManagement: true,
    canAccessAcademicManagement: true, // Acesso completo para ver notas e atribuições
    canAccessFinancial: false,
    canAccessReports: true, // Professores podem ver relatórios acadêmicos
    canAccessSettings: false,
...
export function canAccessRoute(path: string, permissions: UserPermissions): boolean {
  ...
  // Rotas de gestão de professores e notas
  if (path.startsWith('/admin/teacher-management')) {
    return permissions.canAccessAcademicManagement; // Professores e notas fazem parte da gestão acadêmica
  }
