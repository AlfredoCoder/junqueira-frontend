...
import {
  useStudentsConfirmedOptimized as useStudentsConfirmed,
  useStudentFinancialDataOptimized as useStudentFinancialData,
  usePaymentsListOptimized as usePaymentsList
} from '@/hooks/usePaymentsOptimized';
import { useGenerateInvoicePDF } from '@/hooks/usePayments';
...
const PagamentosPage = () => {
  ...
  // Função para carregar estatísticas reais dos pagamentos existentes
  const loadStats = useCallback(async () => {
    try {
      // Usar os dados dos pagamentos já carregados para calcular estatísticas
      if (payments && payments.length > 0) {
        ...
        const valorTotal = payments.reduce((sum, p) => sum + (p.preco || 0), 0);
        
        setStats({
          totalPagamentos: payments.length,
          valorTotal: valorTotal,
          pagamentosHoje: pagamentosHoje,
          alunosAtivos: new Set(payments.map(p => p.codigo_Aluno)).size // Alunos únicos
        });
      } else {
        setStats({
          totalPagamentos: 0,
  ...
  useEffect(() => {
    ...
    const filters = {
      ...
      status: selectedTipoServico !== 'all' ? selectedTipoServico : undefined
    };
    
    fetchPayments(paymentsCurrentPage, 10, filters);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [paymentsCurrentPage, debouncedPaymentsSearch, selectedTipoServico]);

  // Handlers
  const handleViewStudent = useCallback(async (student: { codigo: number; nome: string; n_documento_identificacao: string }) => {
    ...
    <div className="space-y-6 ultra-fast-fade">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
        <StatCard
          ...
          value={stats.totalPagamentos.toLocaleString('pt-AO')}
          change={stats.totalPagamentos > 0 ? "Dados reais" : "Sem dados"}
          changeType={stats.totalPagamentos > 0 ? "up" : "neutral"}
          icon={FileText}
          color="text-[#182F59]"
        ...
        <StatCard
          ...
          change={stats.valorTotal > 0 ? "Valor acumulado" : "Sem dados"}
          changeType={stats.valorTotal > 0 ? "up" : "neutral"}
          icon={DollarSign}
        ...
        <StatCard
          ...
          value={stats.pagamentosHoje.toString()}
          change={stats.pagamentosHoje > 0 ? "Hoje" : "Nenhum hoje"}
          changeType={stats.pagamentosHoje > 0 ? "up" : "neutral"}
          icon={Calendar}
        ...
        <StatCard
          ...
          change={stats.alunosAtivos > 0 ? "Confirmados" : "Sem confirmações"}
          changeType={stats.alunosAtivos > 0 ? "up" : "neutral"}
          icon={Users}
          color="text-blue-600"
          bgColor="bg-gradient-to-br from-blue-50 via-white to-blue-50/50"
            ...
            {paymentsLoading ? (
              <div className="text-center py-8">
                <div className="flex items-center justify-center space-x-2">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#182F59] loading-spinner"></div>
                  <span>Carregando página {paymentsCurrentPage}...</span>
                </div>
              </div>
            ) : paymentsError ? (
              <div className="text-center py-8">
                <div className="flex flex-col items-center space-y-2 ultra-fast-fade">
                          ...
                          <div className="flex items-center justify-center space-x-2">
                            ...
                            >
                              {downloadingPaymentId === payment.codigo ? (
                                <>
                                  <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600 mr-1 loading-spinner"></div>
                                  Gerando...
                                </>
                              ) : (
                                <>
            ...
            <div className="border rounded-lg overflow-hidden max-h-96 overflow-y-auto">
              {studentsLoading ? (
                <div className="p-8 text-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto loading-spinner"></div>
      ...
      <CreditNoteModal
        ...
        onSuccess={() => {
          ...
          fetchPayments(paymentsCurrentPage, 10, {
