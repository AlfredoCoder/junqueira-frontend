...
import { 
  ...
  useAlunoCompleto,
  useCategorias,
  useTiposServicosCompativeis,
  usePropinaAutomatica,
...
const NovoPaymentModal: React.FC<NovoPaymentModalProps> = ({ open, onClose }) => {
  ...
  const extractAcademicData = useCallback((alunoCompleto: any) => {
    ...
    if (confirmacao?.turma) {
      // Usar dados reais da API, não mapeamento local
      return {
        curso: confirmacao.turma.tb_cursos?.designacao || 'Curso não especificado',
        classe: confirmacao.turma.tb_classes?.designacao || 'Classe não especificada',
    ...
    if (turma) {
      // Usar dados reais da API, não mapeamento local
      return {
        curso: turma.tb_cursos?.designacao || 'Curso não especificado',
        classe: turma.tb_classes?.designacao || 'Classe não especificada',
    ...
    // FALLBACK: Dados da API /aluno/completo
    console.log('📋 [extractAcademicData] Usando dados da API completa:', alunoCompleto);
    console.log('📋 [extractAcademicData] Curso encontrado:', alunoCompleto?.dadosAcademicos?.curso);
    const resultado = {
      curso: alunoCompleto?.dadosAcademicos?.curso || 'Curso não especificado',
      classe: alunoCompleto?.dadosAcademicos?.classe || 'Classe não especificada',
      turma: alunoCompleto?.dadosAcademicos?.turma || 'Turma não especificada',
    ...
    console.log('📋 [extractAcademicData] Resultado final:', resultado);
    return resultado;
  }, []);
  ...
  // Estados para seleção inteligente por categoria
  const [categoriaSelecionada, setCategoriaSelecionada] = useState<number | null>(null);
  const [showCategoriaStep, setShowCategoriaStep] = useState(false);
  const [showTiposStep, setShowTiposStep] = useState(false);
  const [categoriaRequerMeses, setCategoriaRequerMeses] = useState(true);
  const [propinaAutomatica, setPropinaAutomatica] = useState<any>(null);
  ...
  // Novos hooks para seleção inteligente
  const { categorias, loading: categoriasLoading, fetchCategorias } = useCategorias();
  const { 
    tiposServicos: tiposCompativeis, 
    loading: tiposCompativeisLoading, 
    alunoInfo,
    fetchTiposCompativeis,
    clearTipos 
  } = useTiposServicosCompativeis();
  const {
    propina: propinaAuto,
    loading: propinaAutoLoading,
    alunoInfo: alunoInfoPropina,
    fetchPropinaAutomatica,
    clearPropina
  } = usePropinaAutomatica();
  ...
  const dadosAcademicos = useMemo(() => {
    ...
    return null;
  }, [alunoCompleto, extractAcademicData]);
  ...
  // Controlar fluxo de seleção quando aluno é selecionado
  useEffect(() => {
    if (selectedAluno) {
      setShowCategoriaStep(true);
      setShowTiposStep(false);
      setCategoriaSelecionada(null);
      clearTipos();
      clearPropina();
      
      // Buscar propina automática para categoria "Propinas"
      fetchPropinaAutomatica(selectedAluno.codigo);
    } else {
      setShowCategoriaStep(false);
      setShowTiposStep(false);
      setCategoriaSelecionada(null);
      clearTipos();
      clearPropina();
  ...
  // Buscar tipos compatíveis quando categoria é selecionada
  useEffect(() => {
    if (selectedAluno && categoriaSelecionada) {
      fetchTiposCompativeis(selectedAluno.codigo, categoriaSelecionada);
      setShowTiposStep(true);
      
      // Determinar se categoria requer seleção de meses
      const categoria = categorias.find(cat => cat.codigo === categoriaSelecionada);
      setCategoriaRequerMeses(categoria?.requerMeses ?? true);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedAluno, categoriaSelecionada, categorias]);

  // Auto-selecionar propina quando categoria "Propinas" for selecionada
  useEffect(() => {
    if (categoriaSelecionada === 1 && propinaAuto && !formData.codigo_Tipo_Servico) {
      setFormData(prev => ({
        ...prev,
        codigo_Tipo_Servico: propinaAuto.codigo,
        preco: propinaAuto.preco?.toString() || ''
      }));
    }
  }, [categoriaSelecionada, propinaAuto, formData.codigo_Tipo_Servico]);

  // Buscar meses pendentes quando categoria Propina for selecionada
  useEffect(() => {
    if (categoriaSelecionada === 1 && selectedAluno && formData.ano) {
      console.log('🔍 Categoria Propina selecionada - buscando meses pendentes...');
      fetchMesesPendentes(selectedAluno.codigo, formData.ano).catch(error => {
        console.error('❌ Erro ao buscar meses pendentes:', error);
      });
    }
  }, [categoriaSelecionada, selectedAluno?.codigo, formData.ano]);
  ...
  const handleSelectAluno = async (aluno: any) => {
    ...
    try {
      ...
      if (confirmacaoRecente) {
        ...
        // NÃO sobrescrever dados corretos da API /aluno/completo
        // Os dados já estão corretos em alunoCompleto.dadosAcademicos
        console.log('✅ [handleAlunoSelect] Mantendo dados corretos da API /aluno/completo');
        console.log('📚 Curso correto da API:', alunoCompleto?.dadosAcademicos?.curso);
        console.log('🎓 Classe correta da API:', alunoCompleto?.dadosAcademicos?.classe);
        console.log('🏫 Turma correta da API:', alunoCompleto?.dadosAcademicos?.turma);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Preparar dados completos para a fatura
      const tipoServicoSelecionado = tiposServico.find(tipo => tipo.codigo === formData.codigo_Tipo_Servico) ||
                                   tiposCompativeis.find(tipo => tipo.codigo === formData.codigo_Tipo_Servico);
      const formaPagamentoSelecionada = formasPagamento.find(forma => forma.codigo === formData.codigo_FormaPagamento);
      
      // Obter dados do usuário logado para operador
      ...
      try {
        ...
        console.error('Erro ao obter nome do operador:', error);
      }

      // Usar o primeiro pagamento para o modal de fatura
      setCreatedPayment({
        codigo: payments[0].codigo || Date.now(),
        fatura: payments[0].fatura || `FAT_${Date.now()}`,
        data: new Date().toISOString(),
        preco: parseFloat(formData.preco) * formData.mesesSelecionados.length,
        aluno: {
          nome: selectedAluno?.nome || "Aluno não identificado",
          codigo: selectedAluno?.codigo || 0,
          curso: alunoInfo?.curso || "Curso não especificado",
          classe: alunoInfo?.classe || "Classe não especificada", 
          turma: alunoInfo?.turma || "Turma não especificada"
        },
        tipoServico: {
          designacao: tipoServicoSelecionado?.designacao || "Serviço não especificado"
        },
        formaPagamento: {
          designacao: formaPagamentoSelecionada?.designacao || "DINHEIRO"
        },
        observacao: formData.observacao,
            ...
            <body>
              <div class="header">
               <img src="/assets/images/icon.png" alt="Logo" style="width: 40px; height: auto; margin-bottom: 5px;" />
                <h2>Complexo Escolar Privado Abilio junqueira</h2>
      ...
      <Dialog open={open && !showInvoiceModal} onOpenChange={onClose}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Busca de Aluno */}
            <div className="space-y-2 ultra-fast-fade">
                  ...
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                    {alunosLoading ? (
                      <div className="p-3 text-center text-gray-500">
                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-500 mx-auto loading-spinner"></div>
                    ...
                    <div className="space-y-3 ultra-fast-fade">
                      ...
                      {alunoCompleto && dadosAcademicos && (
                        <div className="border-t border-green-200 pt-2">
                          <div className="text-xs space-y-1 ultra-fast-fade">
                      ...
                      {selectedAluno && !alunoCompleto && (
                        <div className="border-t border-green-200 pt-2">
                          <div className="flex items-center justify-center py-1">
                            <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-green-500 mr-2 loading-spinner"></div>
            ...
            {selectedAluno && showCategoriaStep && (
              <div className="space-y-4 ultra-fast-fade">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <User className="w-4 h-4 text-blue-600" />
                    <span className="font-medium text-blue-800">Aluno Selecionado:</span>
                    <span className="text-blue-700">{selectedAluno.nome}</span>
                  </div>
                  {alunoInfo && (
                    <div className="text-sm text-blue-600">
                      📚 {alunoInfo.classe} • 🎓 {alunoInfo.curso} • 👥 {alunoInfo.turma}
                    </div>
                  )}
                </div>

                <div className="space-y-2">
                  <Label htmlFor="categoria">1. Selecione a Categoria do Serviço *</Label>
                  <Select 
                    ...
                    onValueChange={(value) => {
                      ...
                      // Limpar seleção anterior se não for propinas
                      if (categoriaId !== 1) {
                        setFormData(prev => ({ 
                          ...prev, 
                          codigo_Tipo_Servico: null, 
                    ...
                    <SelectContent>
                      {categoriasLoading ? (
                        ...
                        categorias.map((categoria) => (
                          <SelectItem key={categoria.codigo} value={categoria.codigo.toString()}>
                            <div className="flex items-center justify-between w-full">
                              <span>{categoria.designacao}</span>
                              {categoria.requerMeses && (
            ...
            {/* Propina Automática para Categoria Propinas */}
            {selectedAluno && categoriaSelecionada === 1 && (
              <div className="space-y-2 ultra-fast-fade">
                <Label>2. Propina Automática (Baseada na Classe e Curso)</Label>
                <div className="text-xs text-green-600 mb-2">
                  🎯 Propina selecionada automaticamente para {alunoInfoPropina?.classe} - {alunoInfoPropina?.curso}
                </div>
                
                {propinaAutoLoading ? (
                  <div className="p-3 border rounded-md bg-gray-50">
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-green-500 mx-auto loading-spinner"></div>
                    <p className="text-center text-sm text-gray-500 mt-2">Buscando propina automática...</p>
                  </div>
                ) : propinaAuto ? (
                  <Card className="border-green-200 bg-green-50">
                    <CardContent className="p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="font-medium text-green-800">{propinaAuto.designacao}</div>
                          <div className="text-sm text-green-600">
                            Preço: {propinaAuto.preco?.toLocaleString('pt-AO')} Kz/mês
                          </div>
                        </div>
                        <Badge variant="secondary" className="bg-green-100 text-green-800">
                  ...
                  <div className="p-3 border rounded-md bg-yellow-50 border-yellow-200">
                    <p className="text-yellow-700 text-sm">
                      ⚠️ Nenhuma propina encontrada para esta classe e curso
                    </p>
                  </div>
                )}
              </div>
            ...
            {/* Seleção de Tipo de Serviço Compatível */}
            {selectedAluno && showTiposStep && categoriaSelecionada !== 1 && (
              <div className="space-y-2 ultra-fast-fade">
                <Label htmlFor="tipo-servico">2. Tipo de Serviço Compatível *</Label>
                <div className="text-xs text-blue-600 mb-2">
                  🎯 Serviços compatíveis com a classe e curso do aluno
                </div>
                
                {tiposCompativeisLoading ? (
                  <div className="p-3 border rounded-md bg-gray-50">
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-500 mx-auto loading-spinner"></div>
                    <p className="text-center text-sm text-gray-500 mt-2">Buscando tipos de serviços compatíveis...</p>
                  </div>
                  ...
                  >
                    ...
                    <SelectContent>
                      {tiposCompativeis.map((tipo) => (
                        <SelectItem key={tipo.codigo} value={tipo.codigo.toString()}>
                          <div className="flex flex-col">
                            <span className="font-medium">{tipo.designacao}</span>
                            <span className="text-sm text-gray-500">
                              {tipo.preco?.toLocaleString('pt-AO')} Kz
                              {tipo.codigo_Classe && tipo.codigo_Curso && ' • Específico para classe+curso'}
                              {tipo.codigo_Classe && !tipo.codigo_Curso && ' • Específico para classe'}
                              {!tipo.codigo_Classe && tipo.codigo_Curso && ' • Específico para curso'}
                              {!tipo.codigo_Classe && !tipo.codigo_Curso && ' • Geral'}
                            </span>
                          </div>
                        </SelectItem>
                      ))}
                  ...
                  <div className="p-3 border rounded-md bg-yellow-50 border-yellow-200">
                    <p className="text-yellow-700 text-sm">
                      ⚠️ Nenhum tipo de serviço compatível encontrado para esta categoria
                    </p>
                  </div>
                )}
              </div>
            ...
            {/* Seleção de Meses Pendentes - Apenas para categorias que requerem meses */}
            {selectedAluno && categoriaRequerMeses && categoriaSelecionada && (
              <div className="space-y-4 ultra-fast-fade">
            ...
            <div className="space-y-2 ultra-fast-fade">
              <Label htmlFor="preco">
                {categoriaRequerMeses ? 'Valor por Mês (Kz) *' : 'Valor Total (Kz) *'}
              </Label>
              <div className="relative">
                <Input
                  id="preco"
              ...
              >
                ...
                <SelectContent>
                  {formasLoading ? (
                    ...
                    formasPagamento
                      .filter(forma => 
                        forma.designacao === 'Dinheiro' || 
                        forma.designacao === 'Depósito Bancário' ||
                        forma.designacao === 'Multicaixa Express'
                      )
                      .map((forma) => (
                        <SelectItem key={forma.codigo} value={forma.codigo.toString()}>
                          {forma.designacao === 'Multicaixa Express' ? 'TPA' : forma.designacao}
                        </SelectItem>
                      ))
                  )}
                </SelectContent>
