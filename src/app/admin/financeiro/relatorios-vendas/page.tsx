...
import { useRelatoriosVendasOptimized as useRelatoriosVendas } from '@/hooks/useRelatoriosVendasOptimized';
import api from '@/utils/api.utils';

const RelatoriosVendasPage = () => {
  const [relatorioFuncionarios, setRelatorioFuncionarios] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    // Carregar relatório de funcionários para hoje
    const hoje = new Date().toISOString().split('T')[0];
    fetchRelatorioFuncionarios(hoje, hoje);
  }, [fetchRelatorioGeral]);
  ...
  const fetchRelatorioFuncionarios = async (dataInicio?: string, dataFim?: string) => {
    ...
    try {
      ...
      const response = await api.get('/api/financial-services/relatorio-vendas-funcionarios', { params });
      setRelatorioFuncionarios(response.data.data);
    } catch (error) {
      console.error('Erro ao buscar relatório de funcionários:', error);
      setErrorFuncionarios('Erro ao carregar relatório de funcionários');
    } finally {
  ...
  const handleBuscarRelatorio = () => {
    if (tipoFiltro === 'personalizado' && dataInicio && dataFim) {
      fetchRelatorioGeral('diario');
      fetchRelatorioFuncionarios(dataInicio, dataFim);
      ...
      if (periodo === 'mensal') {
        ...
        fetchRelatorioGeral('mensal');
        fetchRelatorioFuncionarios(inicioStr, fimStr);
      } else {
        fetchRelatorioGeral(periodo);
        // Para período rápido, usar data de hoje
        const hoje = new Date().toISOString().split('T')[0];
        fetchRelatorioFuncionarios(hoje, hoje);
      }
    }
  ...
  const handleExportarPDF = () => {
    if (!relatorioFuncionarios) return;
    
    // Criar dados para o PDF
    const dadosPDF = {
      titulo: `Relatório de Vendas por Funcionário - ${periodo.charAt(0).toUpperCase() + periodo.slice(1)}`,
      periodo: periodo,
      dataInicio: formatarData(relatorioFuncionarios.resumo.periodo.dataInicio),
      dataFim: formatarData(relatorioFuncionarios.resumo.periodo.dataFim),
      totalGeral: relatorioFuncionarios.resumo.totalGeral,
      totalPagamentos: relatorioFuncionarios.resumo.totalPagamentos,
      funcionarios: relatorioFuncionarios.rankingFuncionarios,
      resumo: relatorioFuncionarios.resumo
    };
        ...
        <body>
          ...
          <div class="resumo">
            ...
            <div class="resumo-card">
              ...
              <p>${dados.resumo.funcionariosAtivos}</p>
            </div>
            <div class="resumo-card">
              <h3>Média por Funcionário</h3>
              <p>${dados.resumo.mediaPorFuncionario.toLocaleString('pt-AO', { minimumFractionDigits: 2 })} Kz</p>
            </div>
          </div>
            ...
            <tbody>
              ${dados.funcionarios.map((func: any, index: number) => `
                <tr>
                  ...
                  <td>${func.nome}</td>
                  <td>${func.totalVendas}</td>
                  <td class="valor">${func.valorTotal.toLocaleString('pt-AO', { minimumFractionDigits: 2 })} Kz</td>
                  <td class="valor">${((func.valorTotal / dados.totalGeral) * 100).toFixed(1)}%</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={!relatorioFuncionarios || loadingFuncionarios}
      ...
      {/* Resumo Geral */}
      {(relatorioFuncionarios || relatorioGeral) && !loadingFuncionarios && !loading && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">
                      {formatarMoeda(relatorioFuncionarios?.resumo?.totalGeral || 0)} Kz
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-blue-600">
                      {relatorioFuncionarios?.resumo?.totalPagamentos || 0}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-purple-600">
                      {relatorioFuncionarios?.resumo?.funcionariosAtivos || 0}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-orange-600">
                      {formatarMoeda(relatorioFuncionarios?.resumo?.mediaPorFuncionario || 0)} Kz
                    </p>
                  </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600">
                    {formatarData(relatorioFuncionarios?.resumo?.periodo?.dataInicio)} até {formatarData(relatorioFuncionarios?.resumo?.periodo?.dataFim)}
                  </p>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              {relatorioFuncionarios?.rankingFuncionarios?.length > 0 ? (
                <div className="space-y-4">
                  {relatorioFuncionarios.rankingFuncionarios.map((funcionario: any, index: number) => (
                    <div key={funcionario.codigo} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            ID: {funcionario.codigo} • {funcionario.totalVendas} pagamentos
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-lg text-green-600">
                          {formatarMoeda(funcionario.valorTotal)} Kz
                        </p>
                        <p className="text-sm text-gray-600">
                          {((funcionario.valorTotal / (relatorioFuncionarios?.resumo?.totalGeral || 1)) * 100).toFixed(1)}% do total
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>Nenhum funcionário encontrado no período selecionado</p>
                  <p className="text-sm">Tente ajustar os filtros de período</p>
                </div>
              )}
            </CardContent>
          </Card>
