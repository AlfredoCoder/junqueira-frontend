...
interface NotaDisciplina {
  ...
  trimestre1: NotaTrimestre;
  trimestre2: NotaTrimestre;
  trimestre3: NotaTrimestre;
...
export default function PerfilAlunoPage() {
  ...
  useEffect(() => {
    ...
    if (perfil?.estadoFinanceiro?.mesesPendentes && perfil.estadoFinanceiro.mesesPendentes.length > 0) {
      ...
      console.log('🔒 [PERFIL FRONTEND] Status contencioso local atualizado:', status);
      console.log('📅 [PERFIL FRONTEND] Data atual:', new Date().toLocaleDateString());
      console.log('🎯 [PERFIL FRONTEND] OUTUBRO deveria estar em contencioso (após 15/10/2025)');
    } else {
      console.log('⚠️ [PERFIL FRONTEND] Nenhum mês pendente encontrado ou array vazio');
      setStatusContenciosoLocal({
        emContencioso: false,
        mesesContenciosos: [],
        diasRestantes: 0,
        podeVerNotas: true
  ...
  // Helper para obter dados do usuário baseado no tipo
  const getDadosUsuario = () => {
    ...
    if (tipoUsuario === 'Professor' && perfil.professor) {
      return {
        ...
        email: perfil.professor.email || '',
        telefone: perfil.professor.telefone || '',
        tipo: 'Professor',
        dadosEspecificos: perfil.professor
      };
    } else if (perfil.aluno) {
      return {
        nome: perfil.aluno.nome,
        email: perfil.aluno.email || '',
        telefone: perfil.aluno.telefone || '',
        tipo: 'Aluno',
        dadosEspecificos: perfil.aluno
      };
    }
  ...
  const carregarPerfil = async () => {
    try {
      ...
      } else if (tipoUsuario === 'Administrador' || tipoUsuario === 'Secretaria' || tipoUsuario === 'Diretor') {
        // Para outros tipos, pode usar uma API genérica ou específica
        apiEndpoint = '/api/aluno/perfil'; // Temporário, pode ser ajustado
      }
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {(() => {
                  ...
                  if (!dadosUsuario) return <p>Dados não disponíveis</p>;
                  
                  return (
                    <>
                      <div>
                        <Label className="text-sm font-medium text-gray-700">Nome Completo</Label>
                        <p className="text-gray-900 font-medium">{dadosUsuario.nome}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-gray-900">{dadosUsuario.tipo}</p>
                      </div>
                      {dadosUsuario.email && (
                        <div>
                          <Label className="text-sm font-medium text-gray-700">Email</Label>
                          <p className="text-gray-900">{dadosUsuario.email}</p>
                        </div>
                      )}
                      {dadosUsuario.telefone && (
                        <div>
                          <Label className="text-sm font-medium text-gray-700">Telefone</Label>
                          <p className="text-gray-900">{dadosUsuario.telefone}</p>
                        </div>
                      )}
                    </>
                  );
                      ...
                      return (
                        <tr key={disciplina.codigo} className="hover:bg-gray-50">
                          ...
                          <td className="border border-gray-300 p-2 text-center text-xs font-semibold">
                            <span className={notaDisciplina?.aprovacao === 'Aprovado' ? 'text-green-600 bg-green-100 px-2 py-1 rounded' : notaDisciplina?.aprovacao === 'Reprovado' ? 'text-red-600 bg-red-100 px-2 py-1 rounded' : 'text-gray-500'}>
                              {notaDisciplina?.aprovacao || '-'}
                            </span>
                          </td>
                        </tr>
                      );
