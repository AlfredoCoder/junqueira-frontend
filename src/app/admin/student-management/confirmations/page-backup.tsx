...
export default function ConfirmationsListPage() {
  ...
  // Hooks da API
  const { confirmations: allConfirmations, loading, error, refetch } = useConfirmations();
  const { statistics, loading: statsLoading } = useConfirmationsStatistics();
  const { confirmation: confirmationDetail, loading: detailLoading } = useConfirmation(selectedConfirmation?.codigo);
  const { deleteConfirmation, loading: deleteLoading } = useDeleteConfirmation();

  // Filtrar e paginar confirmações
  const filteredConfirmations = useMemo(() => {
    if (!allConfirmations) return [];
    
    return allConfirmations.filter((confirmation: any) => {
      // Filtro de busca
      if (debouncedSearchTerm) {
        const searchLower = debouncedSearchTerm.toLowerCase();
        const matchesSearch = 
          confirmation.tb_matriculas?.tb_alunos?.nome?.toLowerCase().includes(searchLower) ||
          confirmation.tb_turmas?.designacao?.toLowerCase().includes(searchLower) ||
          confirmation.classificacao?.toLowerCase().includes(searchLower);
      ...
      // Filtro de status
      if (statusFilter !== "all") {
        if (statusFilter === "ativo" && confirmation.codigo_Status !== 1) return false;
        if (statusFilter === "inativo" && confirmation.codigo_Status !== 0) return false;
      }
      
      return true;
    });
  }, [allConfirmations, debouncedSearchTerm, statusFilter]);
  ...
  }, [filteredConfirmations, currentPage, itemsPerPage]);
  ...
  const handleEditConfirmation = useCallback((confirmationId: number) => {
    ...
    <div className="space-y-6 ultra-fast-fade">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
        ...
        {/* <StatCard
          ...
          accentColor="bg-gradient-to-br from-[#FFD002] to-[#F5DEB3]"
        /> */}
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              {isLoading && <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-[#F9CD1D] loading-spinner"></div>}
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {isLoading && !confirmations?.length ? (
            <div className="flex items-center justify-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#F9CD1D] loading-spinner"></div>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center space-x-2">
              ...
              {confirmationLoading && <Loader2 className="h-4 w-4 animate-spin loading-spinner" />}
            </DialogTitle>
          </DialogHeader>
          
          {confirmationLoading ? (
            <div className="flex items-center justify-center py-8">
              <Loader2 className="h-8 w-8 animate-spin text-blue-600 loading-spinner" />
