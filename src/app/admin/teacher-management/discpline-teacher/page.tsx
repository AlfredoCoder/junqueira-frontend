...
export default function AtribuicoesProfessoresPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validações
      if (!formData.professorId || !formData.disciplinaId || !formData.cursoId || !formData.anoLectivo) {
        setMessage({ type: 'error', text: 'Todos os campos obrigatórios devem ser preenchidos' });
        return;
      }

      // Usar nova API de atribuição completa
      const atribuicaoData = {
        ...
        // Incluir turma se foi especificada
        ...(formData.incluirTurma && formData.turmaId && { turmaId: parseInt(formData.turmaId) })
      };
      ...
      if (response.ok) {
        ...
        if (data.success) {
          toast.success(data.message || 'Atribuição criada com sucesso!');
          setShowModal(false);
          resetForm();
        } else {
        ...
        setMessage({ type: 'error', text: errorData.message || 'Erro ao criar atribuição' });
      }
