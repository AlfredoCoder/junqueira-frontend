...
export default function AddTurmaPage() {
  ...
  // Debug dos cursos (apenas em desenvolvimento)
  useEffect(() => {
  ...
  }, [courses.length, coursesLoading, coursesError]);
  ...
  useEffect(() => {
    ...
    const loadEssentialData = async () => {
      try {
        ...
        await Promise.all([
          fetchCourses(),
          fetchClasses(1, 50) // Reduzir limite inicial
        ]);
        
        // Dados secundários depois
        setTimeout(() => {
          fetchSalas(1, 50);
          fetchPeriodos(1, 50);
          fetchAnosLectivos(1, 50);
        }, 100);
      } catch (error) {
  ...
  // Memoizar dados filtrados para evitar re-computação
  const availableCourses = useMemo(() => {
    return courses.filter(course => course.codigo_Status === 1);
  }, [courses]);

  const availableClasses = useMemo(() => {
    return classes; // Retornar todas as classes por enquanto
  }, [classes]);

  const availableSalas = useMemo(() => {
    return salas; // Retornar todas as salas por enquanto
  }, [salas]);
                    ...
                    render={({ field }) => (
                      ...
                      >
                        <SelectTrigger className={`h-12 ${errors.codigo_Classe ? 'border-red-500' : ''}`}>
                          {classesLoading && <Loader2 className="w-4 h-4 mr-2 animate-spin loading-spinner" />}
                    ...
                    render={({ field }) => (
                      ...
                      >
                        <SelectTrigger className={`h-12 ${errors.codigo_Sala ? 'border-red-500' : ''}`}>
                          {salasLoading && <Loader2 className="w-4 h-4 mr-2 animate-spin loading-spinner" />}
                    ...
                    render={({ field }) => (
                      ...
                      >
                        <SelectTrigger className={`h-12 ${errors.codigo_Periodo ? 'border-red-500' : ''}`}>
                          {periodosLoading && <Loader2 className="w-4 h-4 mr-2 animate-spin loading-spinner" />}
                    ...
                    render={({ field }) => (
                      ...
                      >
                        <SelectTrigger className="h-12">
                          {anosLoading && <Loader2 className="w-4 h-4 mr-2 animate-spin loading-spinner" />}
